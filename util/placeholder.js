...
(function ($, undefined) {
    $.fn.placeholder = function (options) {
        ...
        $(this).each(function () {
            ...
            // 三种情况打酱油
            // ① 没有placeholder属性值
            // ② value模拟，同时是支持placeholder属性的浏览器,同时不是ie浏览器
            // ③ label模拟，但是无需跨浏览器兼容，同时是支持placeholder属性的浏览器
            var navigatorName = "Microsoft Internet Explorer";
            var whetherIE = !(!!+[1,] && !!window.addEventListener && navigator.appName != navigatorName);
            if (!placeholder || (!params.labelMode && isPlaceholder && !whetherIE) || (params.labelMode && !params.labelAcross && isPlaceholder)) {
                return;
            }
